...
messyverse/
  ...
  loesungen/           # public Verifikations-Layer, Klartext-Soll + Integritaets-Digest (P2)
```
...
- `loesungen/` (E15, erledigt; präzisiert in E15(2) / Review 2026-06-23 E5): öffentlich als Verifikations-Layer mit Klartext-Soll (ohnehin aus der öffentlichen Welt herleitbar), abgesichert durch einen Integritäts-Digest als Sekundär-Check. Detail in den P2-Akzeptanzkriterien.
- Übungsumfang Tag 2 (erledigt): vier Kern-Aufgaben (PDF-Extraktion, ISBN→Open Library, Datei-Sortieren, DOI→Crossref mit Pagination) plus eine niedrigschwellige KI-Klassifikation. Die Klassifikation bleibt bewusst klein — kleines kontrolliertes Vokabular, Constraint-Prüfung statt exaktem Soll — zeit- und zielgruppengerecht für 3,5 Stunden.
- ISBN-/DOI-Auswahl (erledigt): von mir kuratiert nach der Werte- und Diversitäts-Leitplanke (`WELT.md`); rund 10 bis 13 klassische Titel, ISBNs/DOIs in P1 live gegen Open Library/Crossref verifiziert und gepinnt, Vorlage zur Freigabe vor dem Generieren.
...
- schlagworte-Kanonisierung (E8): Das Verschlagwortungs-Mapping zeigt auf `katalog/schlagworte.csv` statt auf einen nicht-kanonischen Ordner `schlagworte/`. Diese CSV ist zugleich das kontrollierte Vokabular (E12), klein gehalten für die niedrigschwellige Klassifikation der ersten Welle.
- Kontrolliertes Vokabular und Golden-Schema (E12): Die KI-Klassifikation ist in der ersten Welle, niedrigschwellig gehalten. Das kontrollierte Vokabular `katalog/schlagworte.csv` ist daher ein P1-/Kanon-Artefakt und bleibt klein (~6 bis 8 breite Sachkategorien); geprüft wird per Constraint (alle vergebenen Schlagworte stammen aus dem Vokabular), nicht per exaktem Soll. Pro Übung wird ein Golden-Output-Schema definiert (Golden CSV/JSON/Markdown bzw. `loesungen/`-Manifest), nicht erst als spätere Ablage — verbindet sich mit dem Verifikations-Layer (E15).
- Public-Repo-Hygiene, zweischichtig und fail-closed (E4): Layer 1 ist die `.gitignore` als Typ-Allowlist — versioniert werden nur die kuratierten Top-Level-Dateien und pro Welt-Ordner die deklarierten Inhaltstypen; alles andere (Schlüssel, `.env`, `.yaml`, Datenbanken, endungslose Dateien, die briefer-/Authoring-Infrastruktur) bleibt strukturell ignoriert, ohne Namens-Denylist (umgesetzt). Layer 2 fängt den Rest, den eine Typ-Allowlist prinzipiell nicht fangen kann — ein Secret, das einen erlaubten Typ trägt, etwa ein Key in einer `.json`-Fixture: ein inhaltsbasierter Secret-Scan (gitleaks) als pre-commit-Hook, als CI-Gate vor jedem Merge und als Full-History-Scan vor dem ersten Public-Push. Trennung der Verantwortung: die `.gitignore` bestimmt, welche Dateitypen Projektinhalt sind; der Scan garantiert, dass kein Credential — egal wie benannt — mitfährt. Beide Schichten müssen grün sein, bevor das Repo öffentlich geht.
...
- loesungen/ als öffentlicher Verifikations-Layer mit Klartext-Soll (E15, präzisiert in E15(2) und Review 2026-06-23 E5, PK-Votum A): `loesungen/` ist öffentlich. (1) Das Soll steht im Klartext, weil es ohnehin aus der öffentlichen Welt herleitbar ist — auch beim Datei-Sortieren, denn das Soll-Datum steht im öffentlichen Datei-Inhalt (Zeile „Datum:"). Die Diff-Zelle vergleicht den TN-Output direkt gegen das Klartext-Mapping und benennt Abweichungen namentlich. (2) Ein Integritäts-Digest (`integritaets_sha256`) bleibt als Sekundär-Check gegen versehentliche Veränderung des Solls, nicht als Verschleierung. So bleiben Ein-Repo-SoT und Laufzeit-Self-Check beide erhalten. Historie: Die ursprüngliche E15-Fassung sah für „Ergebnis gleich Lösung" (Datei-Sortieren) einen nicht-invertierbaren Digest vor; E15(2) hat das zugunsten des Klartext-Mappings aufgelöst, weil der Sollwert auch dort öffentlich herleitbar ist.
- Notebook-Schlüssel- und Souveränitäts-Policy (E7): (1) Kein In-Notebook-Secret und kein API-Key nötig — die KI-Verschlagwortung läuft über den eigenen Chat-Assistenten in einem separaten Fenster (Kursbeschreibung „KI-Assistent der eigenen Wahl"). (2) Eine optionale In-Notebook-Variante folgt fixtures-first und nutzt ausschließlich einen souveränen EU-Endpoint (KISSKI/GWDG), nie CN-Cloud. (3) Release-Checkliste: die Notebooks setzen keine Secrets voraus.
